...
const fixture = Array.from({length: 4}, (_, index) => ({
  id: String(index + 1).repeat(64), name: ['Personal', 'Work', 'Research', '<img src=x>'][index],
...
class FakeClient {
  ...
  copy() { copies++; return Promise.resolve({copied: true}); }
  startSwitch() { switches++; return Promise.resolve({state: 'running'}); }
...
app.whenReady().then(async () => {
  ...
  await waitFor(window, 'document.querySelectorAll(".account").length === 8');
  assert.equal(await window.webContents.executeJavaScript('document.querySelectorAll("img").length'), 0);
  assert.equal(await window.webContents.executeJavaScript('document.querySelector("#remote button").disabled'), true);
  await window.webContents.executeJavaScript('document.querySelector("#local button").click()');
  ...
  await waitFor(window, '!document.querySelector("#refresh").disabled');
  await window.webContents.executeJavaScript('document.querySelectorAll("#remote button")[1].click()');
  ...
  console.log('Accounts window passed: metadata rendering, cancel, copy, switch confirmation, reconnect, and text escaping.');
