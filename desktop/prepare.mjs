...
if(!['26.903.61454','26.911.61220'].includes(installed.version))throw Error(`Desktop ${installed.version} is not yet supported by these patches. No installed app or existing runtime files were changed.`);
const currentBuild=installed.version==='26.911.61220';
...
html=replaceOne(html,currentBuild?/<head>/:/<!-- PROD_CSP_TAG_HERE -->/,`${currentBuild?'<head>':''}<script type="module" src="/assets/preload.js"></script>`,'browser preload');
...
for(const file of fs.readdirSync(path.join(out,'webview/assets'))){
 ...
 if(!currentBuild&&!text.includes('encodingLevel=`structuredClonable`'))continue;
 if(!currentBuild)text=replaceOne(text,/encodingLevel=`structuredClonable`/,'encodingLevel=`jsonCompatible`','viewer port encoding');
...
if(currentBuild){
 ...
 for(const file of fs.readdirSync(path.join(out,'webview/assets'))){
  ...
  const target=path.join(out,'webview/assets',file),source=fs.readFileSync(target,'utf8');
  if(!source.includes('encodingLevel=`structuredClonable`'))continue;
  fs.writeFileSync(target,replaceOne(source,/encodingLevel=`structuredClonable`/,'encodingLevel=`jsonCompatible`','split transport encoding'));transports++;
 }
 if(transports!==2)throw Error(`Expected two split transports; found ${transports}`);
}
